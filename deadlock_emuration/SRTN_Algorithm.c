...
I will implement based on SRTN(First Remaining Time Next) scheduling algorithms, with time quantum of 3.
*/

/**
 * 
 * I have obtained test case from moodle:
 * 
output file of:
P1 0 3 1
P4 0 2 1
...
P4 6 2 2
 */
// ASSUMPTION THAT ALL ARE IN ORDER . there is no exceptions &@($*@())
...
void print_result(PCB_T pcb_t[], int line_max){
    FILE *op_file = fopen("results-task2.txt", "w"); // op stands for output file
    for (int i = 0; i < line_max; i++){
 ...
 * For Task 2, please refer to the specificatio sheet or the user manual submitted together with this file.
 ...
 * It will follow Shortest Remainig Time Next scheduling algorithm, and it is implemented by using the above functions.
 * 
 */
int main(int argc, char *argv[]){
    ...
    for (int i = 0; i < MAX_TIME; i++){
        ...
        if (is_running(pcb_t, pcb_length) == -1 && first_process == 1){  // if no process is currently running
            first_process = 0;
            int srtn = find_shortest(pcb_t, pcb_length);
            if (srtn == -1){
        ...
        if (is_running(pcb_t, pcb_length) == -1 || time_count == TIME_QUANTUM){  // if no process is currently running
            ...
            // printf("%d\n", srtn);
            if (srtn == -1){
                tick_waiting(pcb_t, pcb_length);  // in this exception, go on to next iteration
                // time_count++;
                continue;
            }
            int current_running = is_running(pcb_t, pcb_length);

            if (current_running != srtn){ // only print if running process has forcibly been changed
                int running = make_running(pcb_t, srtn);  // this will make the waiting process to running following SRTN, in order!
                pcb_t[current_running].state = READY; // if interrupted, set the previous process back to ready
                printf("Time %d:    %s is in the running state.\n", i, pcb_t[srtn].process_name);
            }
        }
